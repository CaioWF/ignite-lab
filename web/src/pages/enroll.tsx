...
function Enroll({ data }: EnrollProps) {
  const [loadingCreateEnrollment, setLoadingCreateEnrollment] = useState(false);
  const [createPurchase] = useCreatePurchaseMutation()

  async function handlePurchaseProduct(productId: string) {

    setLoadingCreateEnrollment(true);
    ...
    setLoadingCreateEnrollment(false);
                  ...
                  <li key={product.id}>
                    <div className="px-4 py-4 flex items-center sm:px-6">
                      ...
                      <div className="ml-5 flex-shrink-0">
                        <button disabled={loadingCreateEnrollment} onClick={() => handlePurchaseProduct(product.id)} className="px-2 py-1 border border-transparent text-base font-medium rounded-md text-white bg-violet-600 hover:bg-violet-700">
